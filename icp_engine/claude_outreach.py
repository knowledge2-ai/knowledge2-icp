...
def generate_outreach(
    ...
    *,
    ...
    grounded in the scraped ``evidence`` plus optional K2-retrieved
    ``account_context``, and a forced tool call returns a validated draft. Raises
    ``ClaudeUnavailable`` when the SDK or key is missing so callers can fall back
    ...
    user_prompt = _outreach_prompt(company, persona, evidence, role, account_context)
...
def _outreach_prompt(
    ...
    return (
        ...
        "Evidence (scraped from the company's public web presence):\n"
