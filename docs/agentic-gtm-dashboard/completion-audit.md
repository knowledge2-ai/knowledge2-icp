...
|----------------|--------|----------|------------------------|
| Search internet for companies | Implemented | `icp_engine/discovery.py`, `POST /api/search`, dashboard Discover preview, `deployment/cloudflare/worker.js`, `tests/test_discovery.py`, `tests/test_web.py` | Local Python supports Serper via `SERPER_API_KEY`/`SERP_API_KEY` and falls back to DuckDuckGo HTML search. Cloudflare uses Serper when configured, Apollo company search when only Apollo is configured, then committed seed/manual candidates. Search-result profile/resource URLs are associated with discovered company domains. |
...
| Admin UI to configure criteria markdown/prompt | Implemented | Criteria and Setup tabs in `icp_engine/web_assets/index.html`, `POST /api/criteria`, `AppStore.save_criteria`, seeded prompts/settings/lists in `icp_engine/seed_defaults.py`, `tests/test_web.py`, `tests/test_app_store.py` | Writes local app-state criteria, not canonical `icp.md`. Cloudflare normal dashboard flow is public; K2 apply sync remains token-protected. |
...
| Dashboard shows discovered companies and personas | Implemented | Leads and Prospects tabs, `icp_engine/web_assets/seed-companies.json`, `data/ranked_companies.csv`, screenshots `01_leads.jpg`, `02_prospects.jpg`, `make e2e-smoke` | Prospect/persona export available as CSV/JSON. The deployed seed universe contains 428 accounts, including 224 Volaris portfolio companies, 100 Harris portfolio companies, ICP examples such as Mojio, Automate, Hootsuite, and Geotab, plus the 100-company qualified data run led by ServiceTitan, Samsara, and Limble CMMS. |
